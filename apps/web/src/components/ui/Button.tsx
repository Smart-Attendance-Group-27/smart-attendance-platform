...
type ButtonVariant = "default" | "primary" | "danger" | "link";
...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(function Button(
  ...
  return (
    <button
      ref={ref}
      type={type}
      className={`inline-flex min-h-[34px] items-center gap-1.5 rounded-sm px-3 text-xs font-medium disabled:cursor-not-allowed disabled:opacity-50 ${VARIANT_CLASSES[variant]} ${className}`}
      {...props}
    />
  );
});
