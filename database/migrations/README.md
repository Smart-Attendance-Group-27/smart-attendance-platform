...
| `20260925_01_academic_correction_requests` | Adds `academic.correction_requests`, the review queue for lecturer-submitted course-data and timetable correction requests (status, requester, affected offering and timetable entry, reviewer columns for a later admin workflow) | **No.** Not yet run against a throwaway PostgreSQL 16 | **Not applied.** Apply manually after review. |
